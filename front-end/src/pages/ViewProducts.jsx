...
if(productLoading) return <Loading/>
  return (
    <div className="flex flex-col gap-3 contenBody p-1 ">
      <div className="flex gap-2 font-extralight text-red-500 "><NavLink to='/userdash'>Home</NavLink> <p className="text-slate-600 font-thin">{finalCorrectedtitle}</p></div>
         <h1 className="text-slate-600 font-semibold text-xl">{finalCorrectedtitle}</h1>
         <div className="flex gap-5 flex-wrap justify-center">
         {productData?.map(el=>(
          <div  key={el.productid}>
            <DisplayingProducts price={priceData} titles={animal} data={el} loading={productLoading}/>
            </div> ))}
         </div>
    </div>
  )
